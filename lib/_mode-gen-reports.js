
var mkdirp = require('mkdirp'),
fs = require('fs'),
path = require('path'),
marked = require('marked'),
dir = require('node-dir');

// patterns
//var headPat = /---[\s|\S]*---/;
//var mdPat = /.md$/;

// called from main index.js
exports.run = function (options) {

    // always make the .mdfm/reports folder if not there
    mkdirp('.mdfm/reports', function (e) {

        //console.log(options.source);

        // use node-dir to loop over markdown files
        dir.readFiles(

            // find markdown files at options.source
            options.source,

            // options for node-dir
        {
            match: mdPat
        },

            // for each *.md file
            function (err, content, fn, next) {

            require('./_mk-html-file.js').mkFile(fn,content).then(function (html) {

                //console.log(fn);
                next();

            }).catch (function (e) {

                console.log(e);
                next();

            });

            // just writing simple html documents
            /*
            fs.writeFile('.mdfm/reports/' + path.basename(fn).replace(mdPat, '.html'), marked(content.replace(headPat,'')), function (e) {

            next();

            });
             */

            // playing with the lexer
            /*
            var tokens = marked.lexer(content);

            tokens.forEach(function (token) {

            if (token.type === 'paragraph') {

            console.log(token.text.split(' '));

            }

            });
             */

        });

    });

};
